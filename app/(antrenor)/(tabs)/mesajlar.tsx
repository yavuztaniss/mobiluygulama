...
import { getAntrenorKonusmalar } from '../../../src/data/antrenorRepo';
...
export default function AntrenorMesajlarScreen() {
  ...
  const [konusmalar, setKonusmalar] = useState<AntrenorKonusma[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setKonusmalar(await getAntrenorKonusmalar());
    } catch {
...
function createStyles(colors: AppColors) {
  ...
  row: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: colors.navyBorder },
  ...
  time: { fontFamily: fontFamily.manropeBold, fontSize: 10.5, color: colors.textFaint },
