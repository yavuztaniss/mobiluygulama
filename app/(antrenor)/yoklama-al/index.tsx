...
export default function YoklamaAlScreen() {
  ...
  const [aktif, setAktif] = useState<AktifAntrenman | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      // Kilit: yoklama yalnızca ŞU AN devam eden antrenman için açılır — Bugün
      // ekranındaki buton da, doğrudan URL ile gelen de aynı kontrolden geçer.
      const antrenman = await getAktifAntrenman();
      setAktif(antrenman);
      if (!antrenman) {
        setSatirlar([]);
        return;
      }
      const [rows, durum] = await Promise.all([
        getYoklamaSatirlari(antrenman.id),
        getYoklamaKayitDurumu(antrenman.id),
      ]);
      setSatirlar(rows);
  ...
  async function onSave() {
    ...
    setSaving(true);
    try {
      const result = await yoklamaKaydet(aktif.id);
      setOverlay(result);
    } catch {
      showToast('Yoklama kaydedilemedi — bağlantıyı kontrol edin');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function closeOverlay() {
    ...
    if (!aktif) return;
    try {
      const durum = await getYoklamaKayitDurumu(aktif.id);
      setSaved(durum.kaydedildi);
      setSavedAt(durum.zaman);
    } catch {
      // durum tazelenemedi — overlay zaten kapandı, tekrar açılışta yüklenir
    }
  }
  ...
  return (
    <ScreenBackground>
      <SafeAreaView style={styles.flex} edges={['top']}>
        <View style={styles.header}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.headerSub}>
              {aktif ? `${aktif.ad} · ${aktif.saat1} – ${aktif.saat2}${aktif.tesis ? ` · ${aktif.tesis}` : ''}` : 'Antrenman saati bekleniyor'}
            </Text>
          </View>
          {aktif && (
        ...
        {!loading && !error && !aktif && (
          <View style={styles.kilitBox}>
            <Text style={styles.kilitIcon}>🔒</Text>
            <Text style={styles.kilitTitle}>Şu an devam eden antrenman yok</Text>
...
function createStyles(colors: AppColors) {
  ...
  kilitTitle: { fontFamily: fontFamily.archivoBold, fontSize: fontSize.lg, color: colors.textBright, textAlign: 'center' },
  kilitText: { fontFamily: fontFamily.manropeSemi, fontSize: fontSize.base, color: colors.textMuted, textAlign: 'center' },
  kilitBtn: { marginTop: spacing.md, paddingVertical: 12, paddingHorizontal: 24, borderRadius: 14, backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border },
  kilitBtnText: { fontFamily: fontFamily.manropeExtra, fontSize: fontSize.base, color: colors.textBright },
  statsCard: { borderRadius: radius.xxl, backgroundColor: colors.panel, borderWidth: 1, borderColor: colors.border, padding: spacing.md },
  ...
  toggleBtnOutActive: { backgroundColor: colors.dangerSoft },
  toggleIcon: { fontSize: 15, color: colors.textDim },
