...
-- ===========================================================================
-- Kulüp duvarının (0021) İÇİNE ikinci bir restrictive duvar: ŞUBE.
-- Duvar "rol şube yöneticisi DEĞİLSE serbest" biçiminde yazıldı, böylece
-- yönetici/muhasebeci/antrenör/veli davranışı HİÇ değişmiyor.
--
-- ⚠ ENUM DEĞERİ AYNI İŞLEMDE KULLANILAMAZ: `alter type ... add value` ile
--   eklenen değer aynı transaction'da kullanılamaz. Bu yüzden hiçbir yerde
--   `'sube_yoneticisi'::app_role` yazılmıyor — politikalar
--   private.current_profile_role() (TEXT) ile, davet kısıtı `rol::text` ile
--   karşılaştırıyor.
--
-- ⚠ RESTRICTIVE ≠ PERMISSIVE: duvar yalnızca daraltır. Kapılar (4.7) olmadan
--   şube yöneticisi kendi şubesinin sporcusunu bile göremiyordu (ölçüldü: 0).
--
-- ⚠ TÜRETİLEN ŞUBE SECURITY DEFINER'DAN OKUNUYOR: politika içinden RLS'li
--   tabloyu sorgulamak sonsuz özyinelemeye yol açıyordu (ölçüldü:
--   "infinite recursion detected in policy for relation sporcular" —
--   veli_sporcu → sporcular → veli_sporcu).
...
-- ===========================================================================
-- Veli uygulamayı ana ekranına eklediğinde KENDİ kulübünün simgesini görecek.
-- Simge manifest'ten okunuyor, manifest de logoyu bir adresten almak zorunda.
--
-- ⚠ KOVA HERKESE AÇIK OKUNUR — BİLİNÇLİ. Simge oturum AÇILMADAN, giriş
--   ekranında ve telefonun ana ekranında görünmek zorunda. Kapalı kova imzalı
--   URL gerektirirdi; onun süresi dolar ve simge bir gün sessizce kırılır.
--   Sızan bilgi kulübün logosu — zaten tabelasında ve formasında.
--
-- ⚠ YAZMA KULÜBE HAPSEDİLDİ. Politika dosya adına değil YOLUN KENDİSİNE
--   bakıyor (<kulup_id>/logo.png). Ölçüldü: A kulübünün yöneticisi B'nin
--   klasörüne yazamıyor — yazabilseydi B'nin velilerinin ana ekranındaki
--   simgeyi ele geçirebilirdi.
